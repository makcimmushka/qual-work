...
export class AuthenticationService {
  ...
  public register(user: Omit<User, 'id'>): Observable<any> {
    return this.http.post<any>(`${environment.apiUrl}/sign-up`, user)
      .pipe(tap(res => console.log(res)));
  }

  public login(phone: string, password: string): Observable<CurrentUser> {
    return this.http.post<{ token: string }>(`${environment.apiUrl}/sign-in`, { phone, password })
      .pipe(map(res => {
